...
async function getdata(){
  const res = await fetch('http://insane.online:9999/sys/info').then(res=>res.json())
  maxscore = res.score||'无记录'
}
...
var menuState = function(game){
  this.init = function(){
    ...
    this.playerArr = [];
    this.levelArr = [];
    this.game.levelstep = this.game.levelstep || 0;
  };
  this.preload = function(){
    ...
    this.load.spritesheet("level", "./assets/lev.png",50,30);
  ...
  this.create = function(){
    ...
    for (var i=0; i<3; i++){
      this.playerArr[i] = this.game.add.sprite(this.world.centerX, 80, "player", i);
      this.playerArr[i].anchor.set(0.5);
      this.playerArr[i].alpha = 0;
    }
    this.playerArr[this.game.playerStyle].alpha = 1;

    var btn1 = this.game.add.sprite(this.world.centerX-100, 80, "button", 4);
    btn1.anchor.set(0.5);
    btn1.inputEnabled = true;
    btn1.events.onInputDown.add(function(){
      this._setPlayer(-1);
    },this);
    var btn2 = this.game.add.sprite(this.world.centerX+100, 80, "button", 5);
    btn2.anchor.set(0.5);
    ...
    for (var i=0; i<2; i++){
      this.levelArr[i] = this.game.add.sprite(this.world.centerX, 150, "level", i);
      this.levelArr[i].anchor.set(0.5);
      this.levelArr[i].alpha = 0;
    }
    this.levelArr[this.game.levelstep].alpha = 1;

    var btn3 = this.game.add.sprite(this.world.centerX-100, 150, "button", 4);
    btn3.anchor.set(0.5);
    btn3.inputEnabled = true;
    btn3.events.onInputDown.add(function(){
      if (this.game.levelstep == 0) {
        return ;
      }else{
        this._setLevel(1);
        this.game.levelstep =0;
        this.levelArr[0].alpha = 0;
        this.levelArr[1].alpha = 1;
      }
    },this);
    var btn4 = this.game.add.sprite(this.world.centerX+100, 150, "button", 5);
    btn4.anchor.set(0.5);
    btn4.inputEnabled = true;
    btn4.events.onInputDown.add(function(){
      if (this.game.levelstep == 1) {
        return ;
      }
      this._setLevel(-1);
      this.game.levelstep =1;
      this.levelArr[0].alpha = 1;
      this.levelArr[1].alpha = 0;
    },this);
  ...
  this._setLevel = function(go){
    this.levelArr[this.game.levelstep].alpha = 1;
    this.levelArr[this.game.levelstep].x = this.world.centerX;
    this.game.add.tween(this.levelArr[this.game.levelstep]).to({alpha:0,x:this.world.centerX+go*50},200,"Linear",true);

    this.game.levelstep = (((this.game.levelstep - go) % 3) + 3) % 3;

    this.levelArr[this.game.levelstep].alpha = 1;
    this.levelArr[this.game.levelstep].x = this.world.centerX;
    this.game.add.tween(this.levelArr[this.game.levelstep]).from({alpha:0,x:this.world.centerX-go*50},200,"Linear",true);
  };
};
...
var mainState = function(game){
  ...
  this.init = function(){
    ...
    var m = this.game.levelstep == 0?'*':3;
    this.items = {txt:[null,null,null,'无记录'],val:[0,3,m]}; // 游戏数据在一个对象中保存：[分数，生命，瞄准器]
  };
  this.create = function(){
    var back = this.game.add.sprite(0,0,"back");
    back.scale.set(this.game.width/160,this.game.height/280);
    ...
    this.items.txt[3] = this.game.add.text(150, 10, '最高分：'+maxscore, {fontSize:"13px", fill:"#671eff"});

    this.game.input.onDown.add(function(){
      if(!moving && !holding){
        holding=true;
        holdTime=this.game.time.now;
        if(this.items.val[2]>0||this.game.levelstep==0){
          point.x=player.x;
          point.y=player.y;
          point.visible = true;
        }
  ...
  this._overMenu = function(){
    ...
    if (this.items.val[0]>maxscore) {
      this.game.add.text(this.world.centerX, this.world.centerY+150, "恭喜打破最高分", {fontSize:"18px", fill:"red"}).anchor.set(0.5);
      maxscore = this.items.val[0]
      this.items.txt[3].text = this.items.val[0]+''
      setscore(this.items.val[0]+'')
    }
  };
};

async function setscore(score){
  await fetch('http://insane.online:9999/sys/update?score='+score);
  getdata();
